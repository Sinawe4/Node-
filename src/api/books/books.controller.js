...
exports.replace = async (ctx) => {
    ...
    // const result = Joi.validate(ctx.request.body, schema); // 첫번째 파라미터는 검증할 객체이고, 두번째는 스키마입니다.//이제는 이방식 사용안됌
    ...
    try {
        ...
        book = await Book.findByIdAndUpdate(id, ctx.request.body, {
            ...
            useFindAndModify: true   // 이 값이 없으면 ctx.body = book 했을때 업데이트 전의 데이터를 보여줍니다.
        });
    } catch (e) {
        return ctx.throw(500, e);
    }
...
exports.update = async (ctx) => {
    ...
    try {
        //아이디로 찾아서 업데이트
        //파라미터는 (아이디, 별경할 값, 설정) 순이다.
        book = await Book.findByIdAndUpdate(id, ctx.request.body, {
            upsert: false, // upsert 의 기본값은 false 입니다.
